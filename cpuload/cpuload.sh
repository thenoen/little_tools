...
PREV_STATS_LOCATION=/tmp/cpuload.previousStatus


sleepDurationSeconds=$1

previousStats=$(cat $PREV_STATS_LOCATION)

currentStats=$(cat /proc/stat)    
echo $currentStats > $PREV_STATS_LOCATION
...
function processInput {
    ...
    CPU_Percentage=$(awk "BEGIN {printf \"%3.0f\n\", ($totald - $idled)/$totald*100}")
